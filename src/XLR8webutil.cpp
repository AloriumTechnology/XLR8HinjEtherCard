...
void XLR8HinjEtherCard::copyIp (uint8_t *dst, const uint8_t *src) {
    memcpy(dst, src, IP_LEN);
}

void XLR8HinjEtherCard::copyMac (uint8_t *dst, const uint8_t *src) {
    memcpy(dst, src, ETH_LEN);
}

void XLR8HinjEtherCard::printIp (const char* msg, const uint8_t *buf) {
    Serial.print(msg);
    XLR8HinjEtherCard::printIp(buf);
    Serial.println();
}

void XLR8HinjEtherCard::printIp (const __FlashStringHelper *ifsh, const uint8_t *buf) {
    Serial.print(ifsh);
    XLR8HinjEtherCard::printIp(buf);
    Serial.println();
}

void XLR8HinjEtherCard::printIp (const uint8_t *buf) {
...
uint8_t XLR8HinjEtherCard::findKeyVal (const char *str,char *strbuf, uint8_t maxlen,const char *key)
{
    byte found = false;
    uint8_t i=0;
    const char *kp;
...
void XLR8HinjEtherCard::urlDecode (char *urlbuf)
{
    char c;
    char *dst = urlbuf;
    while ((c = *urlbuf) != 0) {
...
void XLR8HinjEtherCard::urlEncode (char *str,char *urlbuf)
{
    char c;
    while ((c = *str) != 0) {
        if (c == ' '||isalnum(c)) {
...
uint8_t XLR8HinjEtherCard::parseIp (uint8_t *bytestr,char *str)
{
    char *sptr;
    uint8_t i=0;
    sptr=NULL;
...
void XLR8HinjEtherCard::makeNetStr (char *resultstr,uint8_t *bytestr,uint8_t len,char separator,uint8_t base)
{
    uint8_t i=0;
    uint8_t j=0;
    while(i<len) {
