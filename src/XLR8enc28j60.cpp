...
XLR8SPIClass XLR8EtherSPI(ETH_SPCR_ADDR, ETH_SPSR_ADDR, ETH_SPDR_ADDR);
XLR8GPIOClass XLR8EtherGPIO(DDRBI_Address, PORTBI_Address, PINBI_Address);

uint16_t XLR8ENC28J60::bufferSize;
bool XLR8ENC28J60::broadcast_enabled = false;
bool XLR8ENC28J60::promiscuous_enabled = false;
...
void XLR8ENC28J60::initSPI () {
    XLR8EtherSPI.begin();
    XLR8EtherSPI.skipArduinoPins();
    XLR8EtherSPI.writeSPCR(bit(SPE) | bit(MSTR));
    XLR8EtherSPI.writeSPSR(XLR8EtherSPI.readSPSR() | (1UL << SPI2X));

}

static void enableChip () {
    cli();
    XLR8EtherGPIO.digitalWrite(selectPin, LOW);
}

static void disableChip () {
    XLR8EtherGPIO.digitalWrite(selectPin, HIGH);
    sei();
}

static void xferSPI (byte data) {
    XLR8EtherSPI.writeSPDR(data);
    while (!(XLR8EtherSPI.readSPSR()&(1<<SPIF)))
        ;
}
...
static byte readOp (byte op, byte address) {
    ...
    if (address & 0x80)
        xferSPI(0x00);
    byte result = XLR8EtherSPI.readSPDR();
    disableChip();
    return result;
}
...
static void readBuf(uint16_t len, byte* data) {
    ...
    if (len != 0) {    
        ...
        XLR8EtherSPI.writeSPDR(0x00);
        while (--len) {
            while (!(XLR8EtherSPI.readSPSR() & (1<<SPIF)))
                ;
            nextbyte = XLR8EtherSPI.readSPDR();
            XLR8EtherSPI.writeSPDR(0x00);
            *data++ = nextbyte;     
        }
        while (!(XLR8EtherSPI.readSPSR() & (1<<SPIF)))
            ;
        *data++ = XLR8EtherSPI.readSPDR();
    }
    disableChip();
}
...
static void writeBuf(uint16_t len, const byte* data) {
    ...
    if (len != 0) {
        ...
        while (--len) {
        	...
        	while (!(XLR8EtherSPI.readSPSR() & (1<<SPIF)))
                ;
            XLR8EtherSPI.writeSPDR(nextbyte);
     	};  
        while (!(XLR8EtherSPI.readSPSR() & (1<<SPIF)))
            ;
    }
...
byte XLR8ENC28J60::initialize (uint16_t size, const byte* macaddr, byte csPin) {
    bufferSize = size;
    if (((XLR8EtherSPI.readSPCR() >> SPE) & 0x01) == 0)
        initSPI();
    selectPin = csPin;
    XLR8EtherGPIO.pinMode(selectPin, OUTPUT);
    disableChip();
...
void XLR8ENC28J60::packetSend(uint16_t len) {
    byte retry = 0;
...
uint16_t XLR8ENC28J60::packetReceive() {
...
void XLR8ENC28J60::copyout (byte page, const byte* data) {
...
void XLR8ENC28J60::copyin (byte page, byte* data) {
    uint16_t destPos = SCRATCH_START + (page << SCRATCH_PAGE_SHIFT);
    if (destPos < SCRATCH_START || destPos > SCRATCH_LIMIT - SCRATCH_PAGE_SIZE)
        return;
...
byte XLR8ENC28J60::peekin (byte page, byte off) {
    byte result = 0;
...
void XLR8ENC28J60::powerUp() {
    writeOp(ENC28J60_BIT_FIELD_CLR, ECON2, ECON2_PWRSV);
    while(!readRegByte(ESTAT) & ESTAT_CLKRDY);
    writeOp(ENC28J60_BIT_FIELD_SET, ECON1, ECON1_RXEN);
}

void XLR8ENC28J60::enableBroadcast (bool temporary) {
    writeRegByte(ERXFCON, readRegByte(ERXFCON) | ERXFCON_BCEN);
    if(!temporary)
        broadcast_enabled = true;
}

void XLR8ENC28J60::disableBroadcast (bool temporary) {
    if(!temporary)
...
void XLR8ENC28J60::enableMulticast () {
    writeRegByte(ERXFCON, readRegByte(ERXFCON) | ERXFCON_MCEN);
}

void XLR8ENC28J60::disableMulticast () {
    writeRegByte(ERXFCON, readRegByte(ERXFCON) & ~ERXFCON_MCEN);
}

void XLR8ENC28J60::enablePromiscuous (bool temporary) {
    writeRegByte(ERXFCON, readRegByte(ERXFCON) & ERXFCON_CRCEN);
    if(!temporary)
        promiscuous_enabled = true;
}

void XLR8ENC28J60::disablePromiscuous (bool temporary) {
    if(!temporary)
...
uint8_t XLR8ENC28J60::doBIST ( byte csPin) {
...
void XLR8ENC28J60::memcpy_to_enc(uint16_t dest, void* source, int16_t num) {
    writeReg(EWRPT, dest);
    writeBuf(num, (uint8_t*) source);
}

void XLR8ENC28J60::memcpy_from_enc(void* dest, uint16_t source, int16_t num) {
    writeReg(ERDPT, source);
    readBuf(num, (uint8_t*) dest);
}

static uint16_t endRam = ENC_HEAP_END; 
uint16_t XLR8ENC28J60::enc_malloc(uint16_t size) {
...
uint16_t XLR8ENC28J60::enc_freemem() {
    return endRam-ENC_HEAP_START;
}

uint16_t XLR8ENC28J60::readPacketSlice(char* dest, int16_t maxlength, int16_t packetOffset) {
