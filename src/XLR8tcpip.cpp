...
static void setMACs (const uint8_t *mac) {
    XLR8HinjEtherCard::copyMac(gPB + ETH_DST_MAC, mac);
    XLR8HinjEtherCard::copyMac(gPB + ETH_SRC_MAC, XLR8HinjEtherCard::mymac);
}

static void setMACandIPs (const uint8_t *mac, const uint8_t *dst) {
    setMACs(mac);
    XLR8HinjEtherCard::copyIp(gPB + IP_DST_P, dst);
    XLR8HinjEtherCard::copyIp(gPB + IP_SRC_P, XLR8HinjEtherCard::myip);
}
...
static boolean is_lan(const uint8_t source[IP_LEN], const uint8_t destination[IP_LEN]) {
    ...
    for(int i = 0; i < IP_LEN; i++)
        if((source[i] & XLR8HinjEtherCard::netmask[i]) != (destination[i] & XLR8HinjEtherCard::netmask[i])) {
            return false;
        }
    return true;
}
...
static uint8_t eth_type_is_arp_and_my_ip(uint16_t len) {
    return len >= 41 && gPB[ETH_TYPE_H_P] == ETHTYPE_ARP_H_V &&
           ...
           memcmp(gPB + ETH_ARP_DST_IP_P, XLR8HinjEtherCard::myip, IP_LEN) == 0;
}
...
static uint8_t eth_type_is_ip_and_my_ip(uint16_t len) {
    return len >= 42 && gPB[ETH_TYPE_H_P] == ETHTYPE_IP_H_V &&
           ...
           (memcmp(gPB + IP_DST_P, XLR8HinjEtherCard::myip, IP_LEN) == 0  //not my IP
            || (memcmp(gPB + IP_DST_P, XLR8HinjEtherCard::broadcastip, IP_LEN) == 0) //not subnet broadcast
...
static void make_eth_ip() {
    ...
    XLR8HinjEtherCard::copyIp(gPB + IP_DST_P, gPB + IP_SRC_P);
    XLR8HinjEtherCard::copyIp(gPB + IP_SRC_P, XLR8HinjEtherCard::myip);
...
static void make_arp_answer_from_request() {
    ...
    XLR8HinjEtherCard::copyMac(gPB + ETH_ARP_DST_MAC_P, gPB + ETH_ARP_SRC_MAC_P);
    XLR8HinjEtherCard::copyMac(gPB + ETH_ARP_SRC_MAC_P, XLR8HinjEtherCard::mymac);
    XLR8HinjEtherCard::copyIp(gPB + ETH_ARP_DST_IP_P, gPB + ETH_ARP_SRC_IP_P);
    XLR8HinjEtherCard::copyIp(gPB + ETH_ARP_SRC_IP_P, XLR8HinjEtherCard::myip);
    XLR8HinjEtherCard::packetSend(42);
}
...
static void make_echo_reply_from_request(uint16_t len) {
    ...
    XLR8HinjEtherCard::packetSend(len);
}

void XLR8HinjEtherCard::makeUdpReply (const char *data,uint8_t datalen,uint16_t port) {
    if (datalen>220)
        datalen = 220;
...
static void make_tcp_synack_from_syn() {
    ...
    XLR8HinjEtherCard::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+4+ETH_HEADER_LEN);
}

uint16_t XLR8HinjEtherCard::getTcpPayloadLength() {
...
static void make_tcp_ack_from_any(int16_t datlentoack,uint8_t addflags) {
    ...
    XLR8HinjEtherCard::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN);
}
...
static void make_tcp_ack_with_data_noflags(uint16_t dlen) {
    ...
    XLR8HinjEtherCard::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlen+ETH_HEADER_LEN);
}

void XLR8HinjEtherCard::httpServerReply (uint16_t dlen) {
...
uint32_t XLR8HinjEtherCard::getSequenceNumber() {
    return getBigEndianLong(TCP_SEQ_H_P);
}

void XLR8HinjEtherCard::httpServerReplyAck () {
...
void XLR8HinjEtherCard::httpServerReply_with_flags (uint16_t dlen , uint8_t flags) {
...
void XLR8HinjEtherCard::clientIcmpRequest(const uint8_t *destip) {
    if(is_lan(XLR8HinjEtherCard::myip, destip)) {
    ...
    gPB[ICMP_IDENT_L_P] = XLR8HinjEtherCard::myip[3]; // last byte of my IP
...
void XLR8HinjEtherCard::ntpRequest (uint8_t *ntpip,uint8_t srcport) {
...
uint8_t XLR8HinjEtherCard::ntpProcessAnswer (uint32_t *time,uint8_t dstport_l) {
...
void XLR8HinjEtherCard::udpPrepare (uint16_t sport, const uint8_t *dip, uint16_t dport) {
    ...
    if ((dip[0] & 0xF0) == 0xE0 || *((unsigned long*) dip) == 0xFFFFFFFF || !memcmp(broadcastip,dip,IP_LEN))
        XLR8HinjEtherCard::copyMac(gPB + ETH_DST_MAC, allOnes);
    gPB[ETH_TYPE_H_P] = ETHTYPE_IP_H_V;
    gPB[ETH_TYPE_L_P] = ETHTYPE_IP_L_V;
    memcpy_P(gPB + IP_P,iphdr,sizeof iphdr);
    gPB[IP_TOTLEN_H_P] = 0;
...
void XLR8HinjEtherCard::udpTransmit (uint16_t datalen) {
    gPB[IP_TOTLEN_H_P] = (IP_HEADER_LEN+UDP_HEADER_LEN+datalen) >> 8;
    gPB[IP_TOTLEN_L_P] = IP_HEADER_LEN+UDP_HEADER_LEN+datalen;
    fill_ip_hdr_checksum();
    gPB[UDP_LEN_H_P] = (UDP_HEADER_LEN+datalen) >>8;
...
void XLR8HinjEtherCard::sendUdp (const char *data, uint8_t datalen, uint16_t sport,
...
void XLR8HinjEtherCard::sendWol (uint8_t *wolmac) {
...
static void client_arp_whohas(uint8_t *ip_we_search) {
    ...
    XLR8HinjEtherCard::copyMac(gPB + ETH_ARP_SRC_MAC_P, XLR8HinjEtherCard::mymac);
    XLR8HinjEtherCard::copyIp(gPB + ETH_ARP_DST_IP_P, ip_we_search);
    XLR8HinjEtherCard::copyIp(gPB + ETH_ARP_SRC_IP_P, XLR8HinjEtherCard::myip);
    XLR8HinjEtherCard::packetSend(42);
}

uint8_t XLR8HinjEtherCard::clientWaitingGw () {
    return !(waitgwmac & WGW_HAVE_GW_MAC);
}

uint8_t XLR8HinjEtherCard::clientWaitingDns () {
...
static uint8_t client_store_mac(uint8_t *source_ip, uint8_t *mac) {
    if (memcmp(gPB + ETH_ARP_SRC_IP_P, source_ip, IP_LEN) != 0)
        return 0;
    XLR8HinjEtherCard::copyMac(mac, gPB + ETH_ARP_SRC_MAC_P);
    return 1;
}
...
void XLR8HinjEtherCard::setGwIp (const uint8_t *gwipaddr) {
    delaycnt = 0; //request gateway ARP lookup
    waitgwmac = WGW_INITIAL_ARP; // causes an arp request in the packet loop
    copyIp(gwip, gwipaddr);
}

void XLR8HinjEtherCard::updateBroadcastAddress()
{
...
static void client_syn(uint8_t srcport,uint8_t dstport_h,uint8_t dstport_l) {
    if(is_lan(XLR8HinjEtherCard::myip, XLR8HinjEtherCard::hisip)) {
        setMACandIPs(destmacaddr, XLR8HinjEtherCard::hisip);
    } else {
        setMACandIPs(gwmacaddr, XLR8HinjEtherCard::hisip);
    }
    gPB[ETH_TYPE_H_P] = ETHTYPE_IP_H_V;
    gPB[ETH_TYPE_L_P] = ETHTYPE_IP_L_V;
    memcpy_P(gPB + IP_P,iphdr,sizeof iphdr);
    ...
    XLR8HinjEtherCard::packetSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN+4);
}

uint8_t XLR8HinjEtherCard::clientTcpReq (uint8_t (*result_cb)(uint8_t,uint8_t,uint16_t,uint16_t),
...
static uint16_t www_client_internal_datafill_cb(uint8_t fd) {
    BufferFiller bfill = XLR8HinjEtherCard::tcpOffset();
    if (fd==www_fd) {
        if (client_postval == 0) {
...
void XLR8HinjEtherCard::browseUrl (const char *urlbuf, const char *urlbuf_varpart, const char *hoststr, void (*callback)(uint8_t,uint16_t,uint16_t)) {
    browseUrl(urlbuf, urlbuf_varpart, hoststr, PSTR("Accept: text/html"), callback);
}

void XLR8HinjEtherCard::browseUrl (const char *urlbuf, const char *urlbuf_varpart, const char *hoststr, const char *additionalheaderline, void (*callback)(uint8_t,uint16_t,uint16_t)) {
...
void XLR8HinjEtherCard::httpPost (const char *urlbuf, const char *hoststr, const char *additionalheaderline, const char *postval, void (*callback)(uint8_t,uint16_t,uint16_t)) {
    client_urlbuf = urlbuf;
    client_hoststr = hoststr;
    client_additionalheaderline = additionalheaderline;
...
static uint16_t tcp_datafill_cb(uint8_t fd) {
    ...
    Stash::extract(0, len, XLR8HinjEtherCard::tcpOffset());
    Stash::cleanup();
    XLR8HinjEtherCard::tcpOffset()[len] = 0;
#if SERIAL
    Serial.print("REQUEST: ");
    Serial.println(len);
    Serial.println((char*) XLR8HinjEtherCard::tcpOffset());
#endif
...
static uint8_t tcp_result_cb(uint8_t fd, uint8_t status, uint16_t datapos, uint16_t datalen) {
    if (status == 0) {
        ...
        result_ptr = (char*) XLR8Ether.buffer + datapos;
        // result_ptr[datalen] = 0;
    }
    return 1;
}

uint8_t XLR8HinjEtherCard::tcpSend () {
    www_fd = clientTcpReq(&tcp_result_cb, &tcp_datafill_cb, hisport);
    return www_fd;
}

const char* XLR8HinjEtherCard::tcpReply (uint8_t fd) {
    if (result_fd != fd)
        return 0;
...
void XLR8HinjEtherCard::registerPingCallback (void (*callback)(uint8_t *srcip)) {
    icmp_cb = callback;
}

uint8_t XLR8HinjEtherCard::packetLoopIcmpCheckReply (const uint8_t *ip_monitoredhost) {
...
uint16_t XLR8HinjEtherCard::accept(const uint16_t port, uint16_t plen) {
    uint16_t pos;
...
uint16_t XLR8HinjEtherCard::packetLoop (uint16_t plen) {
    uint16_t len;

#if ETHERCARD_DHCP
    if(using_dhcp) {
        XLR8Ether.DhcpStateMachine(plen);
    }
#endif
    ...
    if (XLR8Ether.udpServerListening() && gPB[IP_PROTO_P]==IP_PROTO_UDP_V)
    {   //Call UDP server handler (callback) if one is defined for this packet
        if(XLR8Ether.udpServerHasProcessedPacket(plen))
    ...
    {   //Source port is in range reserved (by XLR8HinjEtherCard) for client TCP/IP connections
...
void XLR8HinjEtherCard::persistTcpConnection(bool persist) {
