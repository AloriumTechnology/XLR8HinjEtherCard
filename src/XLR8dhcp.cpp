...
static void send_dhcp_message(uint8_t *requestip) {
    ...
    XLR8HinjEtherCard::udpPrepare(DHCP_CLIENT_PORT,
                          (dhcpState == DHCP_STATE_BOUND ? XLR8HinjEtherCard::dhcpip : allOnes),
                          DHCP_SERVER_PORT);
    ...
    XLR8HinjEtherCard::copyMac(gPB + ETH_DST_MAC, allOnes); //force broadcast mac
    ...
    if (dhcpState == DHCP_STATE_BOUND) {
        XLR8HinjEtherCard::copyIp(dhcpPtr->ciaddr, XLR8HinjEtherCard::myip);
    }
    XLR8HinjEtherCard::copyMac(dhcpPtr->chaddr, XLR8HinjEtherCard::mymac);
    ...
    addBytes(ETH_LEN, XLR8HinjEtherCard::mymac);
    ...
    if (requestip != NULL) {
        ...
        addOption(DHCP_OPT_SERVER_IDENTIFIER, IP_LEN, XLR8HinjEtherCard::dhcpip);
    }
    ...
    XLR8HinjEtherCard::udpTransmit((bufPtr - gPB) - UDP_DATA_P);
}
...
static void process_dhcp_offer(uint16_t len, uint8_t *offeredip) {
    ...
    XLR8HinjEtherCard::copyIp(offeredip, dhcpPtr->yiaddr);
    ...
    do {
        ...
        if (option == DHCP_OPT_SERVER_IDENTIFIER) {
            XLR8HinjEtherCard::copyIp(XLR8HinjEtherCard::dhcpip, ptr);
            break;
        }
        ptr += optionLen;
    } while (ptr < gPB + len);
...
static void process_dhcp_ack(uint16_t len) {
    ...
    XLR8HinjEtherCard::copyIp(XLR8HinjEtherCard::myip, dhcpPtr->yiaddr);
    ...
    do {
        ...
        case DHCP_OPT_SUBNET_MASK:
            XLR8HinjEtherCard::copyIp(XLR8HinjEtherCard::netmask, ptr);
            break;
        case DHCP_OPT_ROUTERS:
            XLR8HinjEtherCard::copyIp(XLR8HinjEtherCard::gwip, ptr);
            break;
        case DHCP_OPT_DOMAIN_NAME_SERVERS:
            XLR8HinjEtherCard::copyIp(XLR8HinjEtherCard::dnsip, ptr);
            break;
...
bool XLR8HinjEtherCard::dhcpSetup (const char *hname, bool fromRam) {
...
void XLR8HinjEtherCard::dhcpAddOptionCallback(uint8_t option, DhcpOptionCallback callback)
{
    dhcpCustomOptionNum = option;
    dhcpCustomOptionCallback = callback;
}

void XLR8HinjEtherCard::DhcpStateMachine (uint16_t len)
{
