...
void Stash::load (uint8_t idx, uint8_t blk) {
    if (blk != bufs[idx].bnum) {
        if (idx == WRITEBUF) {
            XLR8Ether.copyout(bufs[idx].bnum, bufs[idx].bytes);
        ...
        bufs[idx].bnum = blk;
        XLR8Ether.copyin(bufs[idx].bnum, bufs[idx].bytes);
    }
}
...
void Stash::release () {
    while (first > 0) {
        freeBlock(first);
        first = XLR8Ether.peekin(first, 63);
    }
}
...
XLR8HinjEtherCard XLR8Ether;

uint8_t XLR8HinjEtherCard::mymac[ETH_LEN];  // my MAC address
uint8_t XLR8HinjEtherCard::myip[IP_LEN];   // my ip address
uint8_t XLR8HinjEtherCard::netmask[IP_LEN]; // subnet mask
uint8_t XLR8HinjEtherCard::broadcastip[IP_LEN]; // broadcast address
uint8_t XLR8HinjEtherCard::gwip[IP_LEN];   // gateway
uint8_t XLR8HinjEtherCard::dhcpip[IP_LEN]; // dhcp server
uint8_t XLR8HinjEtherCard::dnsip[IP_LEN];  // dns server
uint8_t XLR8HinjEtherCard::hisip[IP_LEN];  // ip address of remote host
uint16_t XLR8HinjEtherCard::hisport = HTTP_PORT; // tcp port to browse to
bool XLR8HinjEtherCard::using_dhcp = false;
bool XLR8HinjEtherCard::persist_tcp_connection = false;
uint16_t XLR8HinjEtherCard::delaycnt = 0; //request gateway ARP lookup

uint8_t XLR8HinjEtherCard::begin (const uint16_t size,
                          const uint8_t* macaddr) {
    uint8_t csPin = 6; // ChipSelect is 6 for Ethernet on a Hinj board
    using_dhcp = false;
#if ETHERCARD_STASH
    Stash::initMap();
#endif
...
bool XLR8HinjEtherCard::staticSetup (const uint8_t* my_ip,
                             const uint8_t* gw_ip,
