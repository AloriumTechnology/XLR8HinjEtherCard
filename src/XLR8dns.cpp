...
static void dnsRequest (const char *hostname, bool fromRam) {
    ...
    if (XLR8Ether.dnsip[0] == 0)
        memset(XLR8Ether.dnsip, 8, IP_LEN); // use 8.8.8.8 Google DNS as default
    XLR8Ether.udpPrepare((DNSCLIENT_SRC_PORT_H << 8) | dnstid_l, XLR8Ether.dnsip, DNS_PORT);
...
static bool checkForDnsAnswer (uint16_t plen) {
    ...
    for (;;) {
        ...
        if (p[1] == DNS_TYPE_A && p[9] == 4) { // type "A" and IPv4
            XLR8Ether.copyIp(XLR8Ether.hisip, p + 10);
            break;
        }
        p += p[9] + 10;
    }
...
bool XLR8HinjEtherCard::dnsLookup (const char* name, bool fromRam) {
